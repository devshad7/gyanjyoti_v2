...
export default function QuizComponent() {
  ...
  useEffect(() => {
    const data = getQuizData(activeSubject)
    setQuizData(data)
  ...
  if (!quizData) {
    return <div className="flex justify-center items-center h-64">Loading quiz data...</div>
  }
        ...
        <div className="mb-6 overflow-x-auto">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-3 lg:grid-cols-6 min-w-max">
            {availableSubjects.map((subject) => (
              <TabsTrigger key={subject.id} value={subject.id} className="text-xs md:text-sm whitespace-nowrap">
                {subject.label}
              </TabsTrigger>
            ))}
          </TabsList>
            ...
            {!quizCompleted ? (
              <div className="space-y-6">
                <div className="text-center mb-6">
                  ...
                  <p className="text-gray-600 text-sm md:text-base">{quizData.description}</p>
                </div>
                ...
                <Card className="mb-6">
                    ...
                    <RadioGroup value={selectedAnswer} className="space-y-3">
                      {currentQuestion.options.map((option, index) => (
                        ...
                        >
                          <RadioGroupItem
                            value={option}
                            id={`option-${subject.id}-${index}`}
                            disabled={isAnswerSubmitted}
                            className="sr-only"
                          />
                          <Label
                            htmlFor={`option-${subject.id}-${index}`}
                  ...
                  <CardFooter className="flex flex-col sm:flex-row justify-between gap-4">
                    ...
                    {!isAnswerSubmitted ? (
                      ...
                      <Button onClick={handleNextQuestion} className="bg-blue-600 hover:bg-blue-700 w-full sm:w-auto">
