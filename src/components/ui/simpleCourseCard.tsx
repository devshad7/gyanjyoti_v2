...
export type SimpleCourseCardProps = {
  slug: string;
  subject?: string;
  classLevel?: string;
  title: string;
      ...
      <div className="relative h-[160px]">
        ...
        <div className="h-54 w-auto bg-blue-100">
          <Link href={`/course/${slug}`} className="block h-full w-full pt-4">
            {imageSrc ? (
              <Image src={imageSrc} alt={title} width={500} height={100} className="w-full h-full object-cover object-center" />
            ) : (
      ...
      <div className="p-3 mt-13 flex-1 flex flex-col">
        ...
        <div className="flex justify-end items-center mt-auto">
          <div className="flex items-center">
            ...
            <span className="ml-1 text-xs text-gray-600">{reviews ?? 0} {reviews === 1 ? "Review" : "Reviews"}</span>
          </div>
        </div>
      </div>
    </div>
